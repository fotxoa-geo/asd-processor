...
def get_mean_std_spectra(spec_mat,use_idx=None,keepdims=None):
...
def calculate_outliers_std(spec_mat, outliers = None, sigma_factor=3):
    ...
    outliers_std = np.any(np.abs(spec_mat-mean)[:,self.good_bands_idx] > sigma_factor*std_dev[:,self.good_bands_idx],axis=1)
    return outliers_std

def calculate_outliers_cossim(spec_mat, outliers = None, threshold=0.985):
    ...
    cos_sim_mean = cos_sim_to_ref(spec_mat[:,self.good_bands_idx],np.squeeze(mean[:,self.good_bands_idx]))
    outliers_cos = cos_sim_mean < threshold # *** Arbitrary threshold need to tune ??
    return outliers_cos

def calculate_outliers(spec_mat, std_sigma_factor = 3, cossim_threshold = 0.985, outlier_list='all', outliers = None):
    #Convenience function for finding outliers from multiple methods
    
    if outlier_list is 'all':
    ...
    if 'std' in outlier_list: #outlier_type == 'all' or outlier_type == 'std':
        ...
        outliers_std = self.calculate_outliers_std(spec_mat, outliers = outliers, sigma_factor = std_sigma_factor)
    ...
    if 'cossim' in outlier_list: #outlier_type == 'all' or outlier_type == 'cossim':
        ...
        outliers_cos = self.calculate_outliers_cossim(spec_mat, outliers = outliers, threshold = cossim_threshold)
        outliers = np.logical_or(outliers,outliers_std)

    print(f'Total outliers: {np.sum(outliers)} (>{sigma_factor}x std dev: {np.sum(outliers_std)}, <{threshold} cos sim: {np.sum(outliers_cos)})')
    return outliers
...
class processor:
    def __init__(self):
        ...
        spectralon_data      = np.loadtxt('./correction_data/spectralon_rfl.txt')
        self.spectralon_wl   = spectralon_data[100:,0]
        self.spectralon_rfl  = spectralon_data[100:,1]
        #assert np.all(self.asd_wavelengths == self.spectralon_wl), 'Spectralon and ASD wavelengths must match!'
        ...
        self.flag_treat_REF_as_RAW = False
        self.known_ref_idx = 0
        self.jump_corr_method  
        self.jump_corr_iterations
        
        
    def load_asd_files(data_dir, file_tag):
        ...
        self.asd_wavelengths = asd_data.wavelengths.copy()
        self.good_bands_idx  = get_good_bands(self.bad_wvl_bnd, self.asd_wavelengths)

        # Load in all asd_data for this file_tag
        self.spectra_mat   = np.zeros((self.n_file,self.n_channels))
        self.reference_mat = spectra_mat.copy()
    ...
    def process_wr_REF():
        ...
        self.ref_group     = np.arange(ref_start_bnd.shape) #What WR group each ref belongs to
        
    def process_wr_REF_as_RAW():
        ...
        ref_same         = np.hstack([False,np.all(np.diff(self.ref_i,axis=0)<=1,axis=1)]) #Same group if 1 index or less away from each other
    ...
    def identify_white_references():

        #Intialize type flag
        #0 = white reference; 1 = field spectra; -1 = outlier; -2 = wref outlier
        self.type_flag = np.zeros(n_file)
        ...
        if asd_data.get_spectra_type() == 'RAW':
            self.process_wr_RAW()
        elif asd_data.get_spectra_type() == 'REF':
        ...
        ref_outliers = calculate_outliers(self.ref_mat)
    ...
    def interpolate_white_references(interp_indices):
    ...
    def process_white_references(interp_method = 'linear'):
                
        self.identify_white_references()
        
        spectra_index  = np.arange(self.spectra_mat.shape[0]) 
        meas_i         = spectra_index[self.type_flag == 1] #Index of measurements
        ref_interp_mat = self.interpolate_white_references(meas_i)
    ...
    def apply_jump_correction():
        meas_idx = self.type_flag == 1
        self.reflectance_mat[meas_idx,:] = asd_jump_correction.apply_jump_correction(self.reflectance_mat[meas_idx,:], self.asd_wavelengths,\
                               jump_corr_method = args.jump_corr_method, iterations=args.jump_corr_iterations)   
        return self.reflectance_mat[meas_idx,:]
    
    def apply_corrections(corr_list = 'all'):
        ...
        if 'jump_corr' in corr_list:
            self.apply_jump_correction()
        return self.reflectance_mat[meas_idx,:]
        
    def calculate_outliers():
        meas_idx = self.type_flag == 1
        #Only calculate outliers for valid measurements
        outliers = calculate_outliers(self.reflectance_mat[meas_idx,:], std_sigma_factor = sigma_factor, cossim_threshold = threshold)
        self.type_flag[meas_idx][outliers] = -1 #Mark outliers
        return outliers
            
    def calculate_final_reflectances():
        meas_idx = self.type_flag == 1
    ...
    def process_asd_files(data_dir, file_tag):
