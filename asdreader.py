# Utilities to read and process ASD files
# Adopted from https://github.com/ajtag/asdreader
# under the MIT License

# Adapted by Regina Eckert and Francisco Ochoa (and XXX add yourself!) Feb 24 2022
...
def parse_gps(gps_field):
    #Currently just returns the tuple and does not process the field
...
def parse_calibration_header(asd, offset, verbose=False):
    header_format = '<b'
    ...
    if verbose:
        print(f'len(asd) {len(asd)}, offset {offset}, difference {len(asd) - offset}, size of header {struct.calcsize(header_format)}, buffer count {buffer_count}')
    ...
    if verbose:
        print(f'Calibration buffer: {calibration_buffer}')
...
class reader:
    def __init__(self, filename, verbose = False):
        ...
        self.calibration_header, offset = parse_calibration_header(self.asd, offset, verbose=verbose)
